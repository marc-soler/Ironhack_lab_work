...
df_fuel_types <- read.csv('../pie_chart_data.csv',
                          header = TRUE, 
                          sep = ',')
# Create a pie chart with ggplot2. For this version we first create a stacked bar chart. (ggplot2 doesn't have a built in pie chart funtion)
...
percentages <- paste0(round(df_fuel_types$Number.of.Cars/sum(df_fuel_types$Number.of.Cars)*100, 1), "%") #paste0 concatenates 2 strings without space
